...
AEnemy::AEnemy():
	...
	AttackR(TEXT("AttackRFast")), 
	AttackRFast(TEXT("AttackR")),
	BaseDamage(12.f),
...
void AEnemy::BeginPlay()
{
	...
	//Ignore the camera for the mesh and capsule
	GetMesh()->SetCollisionResponseToChannel(
...
void AEnemy::StunCharacter(AShooterCharacter* Victim)
{
	if (Victim)
	{
		const float Chance = FMath::FRandRange(0, 1);
		if (Victim->GetStunChance() >= Chance)
		{
			Victim->Stun();
		}
	}
