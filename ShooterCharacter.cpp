	...
	bDeath(false)
{
	...
	//Create a camera boom (oulls in towards the character if there is a collision)
	...
	FollowCamera = CreateDefaultSubobject<UCameraComponent>(TEXT("FollowCamera"));// Attach camera to the end of the boom
	FollowCamera->SetupAttachment(CameraBoom, USpringArmComponent::SocketName); //Camera do not rotate relative to the arm
	FollowCamera->bUsePawnControlRotation = false;

	//Don't rotate when the controller rotates. Controller only affects the camera
...
void AShooterCharacter::BeginPlay()
{
	...
	//Create FInterpLocation sstructs for each interp location. Add to array
	InitializedInterpLocations();
}
...
void AShooterCharacter::LookUp(float Value)
{
	...
	if (bAiming)
	{
		LookUpScaleFactor = MouseAimimngLookUpRate;
	}
	else
	{
...
bool AShooterCharacter::GetBeamEndLocation(const FVector& MuzzleSocketLocation, FHitResult& OutHitResult)
{
	...
	// Perform a second trace for the gun barrel
...
void AShooterCharacter::StartCrosshairBulletFire()
{
	...
	GetWorldTimerManager().SetTimer(
		CrosshairShootTimer,
		this,
		&AShooterCharacter::FinishedCrosshairBulletFire, // this will set bFiringBullet to false after the timer
		ShootTimeDuration);
}

void AShooterCharacter::FinishedCrosshairBulletFire()
{
	bFiringBullet = false;
...
void AShooterCharacter::FireButtonPressed()
{
	...
	// if statement to set the character as death if they got hit by an explosive
	if (bDeath)
	{
		Die();
		return;
...
void AShooterCharacter::EquipWeapon(AWeapon* WeaponToEquip, bool bSwapping)
{
	...
	// Set EquippedWeapon to the newly spwned weapon
...
void AShooterCharacter::SelectedButtonReleased()
{
...
void AShooterCharacter::SendBullet()
{
	...
	if (BarrelSocket)
	{
		...
		if (bBeamEnd)
		{
			...
			if (BeamHitResult.Actor.IsValid() && Cast<IBulletHitInterface>(BeamHitResult.Actor.Get()))
			{
				BulletHitInterface = Cast<IBulletHitInterface>(BeamHitResult.Actor.Get());
...
void AShooterCharacter::FinishReloading()
{
	...
	if (AmmoMap.Contains(AmmoType))
	{
		...
		{
			// fill the magazine
			EquippedWeapon->ReloadAmmo(MagEmptySpcae);
			CarriedAmmo -= MagEmptySpcae;
			AmmoMap.Add(AmmoType, CarriedAmmo);
		}
	}
}
...
void AShooterCharacter::InitializedInterpLocations()
{
...
void AShooterCharacter::HighlightedInventorySlot()
{
...
void AShooterCharacter::AnimDeath()
{
	UAnimInstance* AnimInstance = GetMesh()->GetAnimInstance();
...
void AShooterCharacter::UnHighlightedInventorySlot()
{
...
void AShooterCharacter::Tick(float DeltaTime)
{
	...
	// Hnadle Interpolation for zoom when aiming 
	...
	TraceForItems();
	//Interpolate the capsule hald height vased on crouching/standing
...
void AShooterCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Select", IE_Released, this, &AShooterCharacter::SelectedButtonReleased);
